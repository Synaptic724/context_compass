...
# GEMINI.MD - New Profile Onboarding Contract
...
5) Apply onboarding completion config writes:
   - set `runtime_state.active_default_role` to selected steady-state profile.
   - set `runtime_state.first_time_enabled: false`.
...
- `runtime_state.active_default_role: <selected_role>`
