...
2. Repository policy (`GEMINI.MD` and referenced docs).
3. Explicit user instructions compatible with repository policy.
4. This execution contract.
5. Stylistic preferences.

Hard rule
- If this execution contract conflicts with any `GEMINI.MD` rule,
  `GEMINI.MD` wins. No exceptions.
...
`GEMINI.MD` files are the source for operational gates and procedures.
